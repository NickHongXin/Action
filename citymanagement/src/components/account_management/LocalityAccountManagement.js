  	...
  	render(){
	    return (
	    	<div className={Manager.accountSearch}>
						...
						<tbody>
							{
								this.state.localityAccounts.map((item, idx) => (
									<tr key={idx}>
										<td>{idx + 1 + ((this.state.currentPageNo - 1) * this.state.pageSize)}</td>
				...
				<LocalityAccountEditor isActive={this.state.isDialogActive} 
				 	hideDialog={this.hideOrShowDialog} 
				 	LocalityAccountInfo={this.state.selectedAccount} 
